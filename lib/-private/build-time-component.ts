...
export type ComponentNode = AST.MustacheStatement | AST.BlockStatement;
...
export type BuildTimeComponentFactory = GlimmerObjectFactory<any> & typeof BuildTimeComponent;

export default class BuildTimeComponent extends GlimmerObject {
  tagName: string
  concatenatedProperties: string[]
  classNames: string[]
  ...
  constructor({ tagName = 'div', concatenatedProperties = ['classNames'], classNames = [], ...rest }: BuildTimeComponentOptions) {
    super({
      tagName,
  ...
  toElement() {
    return b.element(this.tagName, this.elementAttributes(), [], []);
  }

  elementAttributes(): AST.AttrNode[] {
    let attrs = [];
    if (this.classNames.length > 0) {
      attrs.push(<AST.AttrNode> buildAttr('class', this.classNames.join(' ')));
    }
    return attrs;
  }
}
